...
const FormSchedule = ({ values, onChange }: { values: any, onChange: any }) => {
  ...
  return (
    <WrapperForm>
      ...
      <DropDown name="placeName" onChange={onChange} options={locations.map((value: { name: string, capacity: number, id: number }) => ({ label:  `${value.name} - Capacidade: ${value.capacity}`, value: value.name }))} value={values?.placeName ?? ''} />
